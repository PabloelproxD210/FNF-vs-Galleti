...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'cookies':
				dialogue = CoolUtil.coolTextFile(Paths.txt('cookies/dialogue'));
			case 'virus':
				dialogue = CoolUtil.coolTextFile(Paths.txt('virus/dialogue'));
			case 'devil':
				dialogue = CoolUtil.coolTextFile(Paths.txt('devil/dialogue'));
		}
		...
		switch(SONG.stage)
		{
			...
			case 'criper':
				{
						...
						var criper:FlxSprite = new FlxSprite(-451, -75).loadGraphic(Paths.image('stage/Stage1', 'galleti'));
						criper.setGraphicSize(Std.int(criper.width * 1));
						criper.updateHitbox();
						criper.antialiasing = true;
						criper.scrollFactor.set(0.9, 0.9);
						criper.active = false;
						add(criper);
			...
			case 'angrycriper':
				{
						defaultCamZoom = 0.7;
						curStage = 'angrycriper';
		
						var criper:FlxSprite = new FlxSprite(-451, -75).loadGraphic(Paths.image('stage/Stage2', 'galleti'));
						criper.setGraphicSize(Std.int(criper.width * 1));
						criper.updateHitbox();
						criper.antialiasing = true;
						criper.scrollFactor.set(0.9, 0.9);
						...
						var lineas:FlxSprite = new FlxSprite(0, 0);
						lineas.screenCenter(X);
						lineas.setGraphicSize(Std.int(lineas.width * 0.6));
						lineas.antialiasing = true;
						lineas.scale.set(1.4, 1.4);
						lineas.frames = Paths.getSparrowAtlas('stage/kkkkkkk', 'galleti');
						lineas.animation.addByPrefix('idle', 'animstage2', 7);
						lineas.animation.play('idle');
			...
			case 'criperdemon':
				{
					defaultCamZoom = 0.7;
					curStage = 'criperdemon';
		
					var criper:FlxSprite = new FlxSprite(-451, -75).loadGraphic(Paths.image('stage/Stage3', 'galleti'));
					criper.setGraphicSize(Std.int(criper.width * 1));
					criper.updateHitbox();
					criper.antialiasing = true;
					criper.scrollFactor.set(0.9, 0.9);
		...
		switch (SONG.player2)
		{
			...
			case 'criperdemon':
				...
				if(FlxG.save.data.distractions)
				{
					var XD = new FlxTrail(dad, null, 1, 24, 0.3, 0.069);
					add(XD);
				}
			case 'pico':
				camPos.x += 600;
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - KE - Galleti " + MainMenuState.kadeEngineVer : ""), 16);
